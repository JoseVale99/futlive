...
export function getLateralOrder(position: string): number {
  ...
  // Perspectiva espejo (vista desde portería contraria):
  // R (derecha del equipo) → izquierda visual de pantalla
  // L (izquierda del equipo) → derecha visual de pantalla
  if (base.startsWith('r')) return 0;
  if (base.startsWith('l')) return 4;

  // Sufijo: -R = derecha del equipo → izquierda visual
  if (suffix === 'r') return 1;
  if (suffix === 'l') return 3;
